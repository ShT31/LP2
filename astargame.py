...
def manhattan_distance(state: List[List[int]], goal_state: List[List[int]]) -> int:
    distance = 0
    for i in range(3):
        for j in range(3):
            if state[i][j] != 0:
...
def solve_8_puzzle(initial_state: List[List[int]], goal_state: List[List[int]]) -> List[Tuple[int, List[List[int]]]]:
        ...
        for neighbor_state in neighbors:
            if tuple(map(tuple, map(tuple, neighbor_state))) not in visited_states:
                neighbor_node = PuzzleNode(
                    ...
                    heuristic=manhattan_distance(neighbor_state, goal_state),
                )
